...
public class Configuration {
    ...
    public Configuration(File f) throws Exception {
        ...
        if (yml.getSection("bits") != null) {
            if (yml.getSection("bits").getStringList() != null) {
                conditions.clear();
                commands.clear();
				for (String command : yml.getSection("bits").getStringList()) {
                ...
                for (Data data : yml.getSection("bits").getChilds()) {
                    if (data instanceof Section) {
                        ...
                        conditions.clear();
                        commands.clear();
						if (id.getChild("conditionals") != null) {
							for (String condition : id.getChild("conditionals").getChild("bits-amount", "").getString().split(";")) {
								conditions.add(new Condition(condition));
							}
						}
						for (String command : id.getChild("effects", new ArrayList<>()).getStringList()) {
							commands.add(new Command(command));
						}
                        bitoptions.add(new BitOption(new ArrayList<>(conditions), new CommandList(new ArrayList<>(commands))));
                    }
        ...
        if (yml.getSection("subscriptions") != null) {
            if (yml.getNonNullSection("subscriptions.effects").getStringList() != null || yml.getNonNullSection("subscriptions.if-gifted").getStringList() != null) {
                ...
                commands.clear();
                ifgifted.clear();
				for (String command : yml.getSection("subscriptions.effects", new ArrayList<>()).getStringList()) {
					commands.add(new Command(command));
				}
				for (String command : yml.getSection("subscriptions.if-gifted", new ArrayList<>()).getStringList()) {
					ifgifted.add(new Command(command));
				}
                ...
                for (Data data : yml.getSection("subscriptions").getChilds()) {
                    if (data instanceof Section) {
                        ...
                        ifgifted.clear();
						if (id.getChild("conditionals") != null) {
							for (String condition : id.getChild("conditionals").getChild("months-amount", "").getString().split(";")) {
								conditions.add(new Condition(condition));
							}
						}
						for (String command : id.getChild("effects", new ArrayList<>()).getStringList()) {
						...
						for (String command : id.getChild("if-gifted", new ArrayList<>()).getStringList()) {
							ifgifted.add(new Command(command));
						}
                        subscriptionoptions.add(new SubscriptionOption(new ArrayList<>(conditions), new CommandList(new ArrayList<>(commands)), new CommandList(new ArrayList<>(ifgifted))));
                    }
        ...
        if (yml.getSection("raids") != null) {
            if (yml.getSection("raids").getStringList() != null) {
                conditions.clear();
                commands.clear();
				for (String command : yml.getSection("raids").getStringList()) {
					commands.add(new Command(command));
				}
                ...
                for (Data data : yml.getSection("raids").getChilds()) {
                    if (data instanceof Section) {
                        ...
                        commands.clear();
						if (id.getChild("conditionals") != null) {
							for (String condition : id.getChild("conditionals").getChild("raids-amount", "").getString().split(";")) {
								conditions.add(new Condition(condition));
							}
						}
						for (String command : id.getChild("effects", new ArrayList<>()).getStringList()) {
		...
		if (yml.getSection("channelpoints") != null) {
			for (Data data : yml.getSection("channelpoints").getChilds()) {
				if (data instanceof Section) {
					Section reward = (Section) data;
					commands.clear();
					for (String command : reward.getStringList()) {
						commands.add(new Command(command));
					}
					try {
        ...
        if (yml.getSection("followers") != null) {
            if (yml.getSection("followers").getStringList() != null) {
                commands.clear();
				for (String command : yml.getSection("followers").getStringList()) {
					commands.add(new Command(command));
				}
		...
		for (String command : yml.getSection("messages.commands-after-normal-message", new ArrayList<>()).getStringList()) {
			nmcmds.add(new Command(command));
		}
		for (String command : yml.getSection("messages.commands-after-subscriber-message", new ArrayList<>()).getStringList()) {
			smcmds.add(new Command(command));
		}
		for (String command : yml.getSection("messages.commands-after-highlighted-message", new ArrayList<>()).getStringList()) {
			hmcmds.add(new Command(command));
		}
		for (String command : yml.getSection("messages.commands-after-ritual-message", new ArrayList<>()).getStringList()) {
			rmcmds.add(new Command(command));
		}
    ...
    public List<BitOption> getBitOptions() {
        return bitoptions;
    }
    ...
    public List<RaidOption> getRaidOptions() {
        return raidoptions;
    }

    public List<UUID> getRewardsUUIDs() {
        return new ArrayList<>(rewardscommands.keySet());
    }

    public CommandList getRewardCommands(UUID reward) {
        return rewardscommands.get(reward);
    }

    public CommandList getFollowersCommands() {
        return followerscommands;
    }

    public MessageCommandList getMessageCommandList() {
        return messagecommandlist;
    }

    public String normalMessageFormat() {
        return msgnmlformat;
    }

    public String ritualMessageFormat() {
        return msgfrstformat;
    }

    public String highlightedMessageFormat() {
        return msghlformat;
    }

    public String subscriberIcon() {
        return subicon;
    }
}
