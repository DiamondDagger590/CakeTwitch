package relampagorojo93.caketwitch.relautils.shared.sql.objects.datamodel.subdatabases;
...
import relampagorojo93.caketwitch.relautils.shared.sql.objects.datamodel.constraints.ForeignConstraint;
import relampagorojo93.caketwitch.relautils.shared.sql.objects.datamodel.constraints.UniqueConstraint;
...
public class SQLiteDatabase extends MySQLDatabase {
    ...
    public SQLiteDatabase(String prefix) {
        super(prefix);
    }
    ...
    public boolean updateTables(String version, SQLObject sql, SQLParser... parsers) {
		...
		if (!sql.execute("CREATE TABLE IF NOT EXISTS information(param VARCHAR(16),value VARCHAR(16),CONSTRAINT PK_information PRIMARY KEY (param));")) {
			return false;
		}
        List<String> tlist = new ArrayList<>();
        try {
			if (!sql.execute("BEGIN IMMEDIATE TRANSACTION;")) {
				throw new Exception();
			}
            ...
            if (!v.equals(version)) {
                ...
                for (String name : check) {
                    if (!name.equals("information")) {
						if (!sql.execute("CREATE TABLE " + name + "_old AS SELECT * FROM " + name + ";")) {
							throw new Exception();
						}
						if (!sql.execute("DROP TABLE IF EXISTS " + name + ";")) {
							throw new Exception();
						}
                        tlist.add(name);
                    }
                ...
                for (String table : tlist) {
                    ...
                    if (cfields != null) {
                        String fields = "";
						while (cfields.next()) {
							fields += (fields.isEmpty() ? "" : ",") + cfields.getString("name");
						}
                        cfields.close();
			...
			if (!v.isEmpty()) {
				sql.execute("UPDATE information SET `value`=? WHERE `param`=?;", new Data(12, version), new Data(12, "version"));
			}
			else {
				sql.execute("INSERT INTO information VALUES(?,?);", new Data(12, "version"), new Data(12, version));
			}
            result = true;
			if (!sql.execute("COMMIT TRANSACTION;")) {
				throw new Exception();
			}
        }
        catch (Exception e) {
			...
			if (update) {
				try {
					...
					for (String table : check) {
						if (table.endsWith("_old") && !sql.execute("DROP TABLE IF EXISTS " + table + ";")) {
							throw new Exception();
						}
					}
					if (!sql.execute("COMMIT TRANSACTION;")) {
        ...
        for (Column c : table.getColumns()) {
            ...
            if (column.getAutoIncrement()) {
				...
				if (column.getDefault() != null && !column.getDefault().isEmpty()) {
					esb.append(" DEFAULT ").append(column.getDefault());
				}
            }
        }
        sb.append(esb);
