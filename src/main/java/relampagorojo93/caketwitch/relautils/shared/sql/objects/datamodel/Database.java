package relampagorojo93.caketwitch.relautils.shared.sql.objects.datamodel;
...
import relampagorojo93.caketwitch.relautils.shared.sql.objects.datamodel.joins.Join;
...
public abstract class Database {
    ...
    public Table getTable(String name) {
		for (Table table : tables) {
			if (table.getName().equals(name) || table.getName().equals(prefix + name)) {
				return table;
			}
		}
        return null;
    ...
    public ResultSet select(SQLObject sql, List<Table> allcolumns, Condition... conditions) {
        return select(sql, allcolumns, new ArrayList<>(), conditions);
    }

    public ResultSet select(SQLObject sql, List<Table> allcolumns, List<Column> columns, Condition... conditions) {
        return select(sql, allcolumns, columns, new ArrayList<>(), conditions);
    }

    public ResultSet select(SQLObject sql, List<Table> allcolumns, List<Column> columns, List<Join> joins, Condition... conditions) {
        return selectDistinct(sql, false, allcolumns, columns, joins, conditions);
    }

    public ResultSet selectDistinct(SQLObject sql, List<Table> allcolumns, Condition... conditions) {
        return selectDistinct(sql, allcolumns, new ArrayList<>(), conditions);
    }

    public ResultSet selectDistinct(SQLObject sql, List<Table> allcolumns, List<Column> columns, Condition... conditions) {
        return selectDistinct(sql, true, allcolumns, columns, new ArrayList<>(), conditions);
    }
