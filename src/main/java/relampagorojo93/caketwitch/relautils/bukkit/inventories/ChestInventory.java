package relampagorojo93.caketwitch.relautils.bukkit.inventories;
...
import relampagorojo93.caketwitch.relautils.bukkit.inventories.objects.Modifiable;
import relampagorojo93.caketwitch.relautils.bukkit.inventories.objects.Slot;
...
public class ChestInventory extends AbstractInventory {
    ...
    public Inventory getInventory() {
        if (size > 0) {
            ...
            updateContent();
			if (background != null) {
				for (int i = 0; i < size && i < background.length; i++) {
					if (background[i] != null && getSlot(i) == null) {
						setSlot(i, new Item(background[i]));
					}
				}
			}
			for (Integer i : new ArrayList<>(slots.keySet())) {
				if (i < size) {
    ...
    public void onDrag(InventoryDragEvent e) {
		for (int slot : e.getRawSlots()) {
			if (!(getSlot(slot) instanceof Modifiable)) {
				e.setCancelled(true);
				break;
			}
		}
