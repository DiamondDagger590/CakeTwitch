package relampagorojo93.caketwitch.yamllib;


import relampagorojo93.caketwitch.yamllib.objects.Comment;
import relampagorojo93.caketwitch.yamllib.objects.Data;
import relampagorojo93.caketwitch.yamllib.objects.Section;
import relampagorojo93.caketwitch.yamllib.objects.Space;
import relampagorojo93.caketwitch.yamllib.objects.Type;
...
public class YAMLFile {
    ...
    public Section setSection(String section, Object data) {
        ...
        if (data != null) {
			if (data instanceof List) {
				...
				actual.setData(data.toString());
			}
			for (Section parent = actual.getParent(); parent != null; parent = parent.getParent()) {
				parent.setData(null);
			}
        }
        return actual;
    ...
    public void removeSection(String section) {
		...
		if (sec != null) {
			for (; sec != null && sec.getChilds().isEmpty(); ) {
    ...
    public Section getNonNullSection(String section) {
        return getSection(section, new Section());
    }

    public Section getSection(String section) {
        return getSection(section, (Section) null);
    }
    ...
    public Section getSection(String section, Section def) {
		...
		for (int i = 0; i < split.length; i++) {
			for (Data data : s.getChilds()) {
				if (data.isSection() && data.asSection().getName().equals(split[i])) {
					s = data.asSection();
					break;
				}
			}
    ...
    private String getCompleteLine(BufferedReader r, String line) throws IOException {
        ...
        if (c == '"' || c == '\'') {
            ...
            while (line.length() < 2 || (last = line.lastIndexOf(c)) < 1 || line.charAt(last - 1) == '\\') {
				if (last > 0 && line.charAt(last - 1) == '\\') {
					System.out.println(line.charAt(last - 1) + String.valueOf(line.charAt(last)));
				}
    ...
    private String readSections(BufferedReader r, Section section) throws IOException {
        ...
        while (true) {
            ...
            int loc = 0;
			while (line.length() > loc && line.charAt(loc) == ' ') {
				loc++;
			}
			if (loc < (section.getLevel() - 1) * 2) {
				break;
			}
            if (section.getLevel() != 0 && loc == (section.getLevel() - 1) * 2) {
				if (!line.substring(loc).isEmpty() && line.substring(loc).charAt(0) == '-') {
					...
					if (section.getData() == null) {
						section.setData(new ArrayList<String>());
					}
					List<String> list = section.getStringList();
					if (list != null) {
						if (!line.isEmpty()) {
							line = getCompleteLine(r, line);
						}
						list.add(line);
					}
					else {
						throw new IOException("Trying to add multiline strings on a section with non-multiline data:\r\n\r\n" + line + "\r\n\r\n");
					}
				}
				else {
					break;
				...
				if (line.isEmpty()) {
					section.addChild(new Space());
				}
				else if (line.charAt(0) == '#') {
					section.addChild(new Comment(line));
				}
				else if (line.charAt(0) == '-') {
					...
					if ((sep = line.indexOf(':')) != -1 && sep > 0 && line.charAt(sep - 1) != '\\') {
						if (section.getData() != null) {
							throw new IOException("Trying to add sections on a section of only one data:\r\n\r\n" + (line.length() > 16 ? line.substring(0, 16) + "..." : line) + "\r\n\r\n");
						}
						...
						if (!sec[1].isEmpty()) {
							if (sec[1].startsWith("[") && sec[1].endsWith("]")) {
								...
								for (String obj : objs) {
									if (!(obj = obj.trim()).isEmpty()) {
										list.add(obj);
									}
								}
								data = list;
							}
							else if (!sec[1].startsWith("#")) {
								data = getCompleteLine(r, sec[1]);
							}
						}
					...
					else if (section.getType() == Type.STRING) {
						section.setData(section.getString() + " " + line);
					}
						...
						if (list.size() > 0) {
							...
							throw new IOException("Unexpected string list initialize:\r\n\r\n" + (line.length() > 16 ? line.substring(0, 16) + "..." : line) + "\r\n\r\n");
						}
					}
					else if (section.getData() != null) {
						throw new IOException("Trying to set a different kind of data to an already initialized section:\r\n\r\n" + (line.length() > 16 ? line.substring(0, 16) + "..." : line) + "\r\n\r\n");
					}
					else {
						throw new IOException("Trying to initialize a section from a wrong site:\r\n\r\n" + (line.length() > 16 ? line.substring(0, 16) + "..." : line) + "\r\n\r\n");
					}
				}
			}
			else {
				throw new IOException("This line is in an incorrect level (level " + loc + " on '" + section.getPath() + "' with level " + section.getLevel() + "):\r\n\r\n" + (line.length() > 16 ? line.substring(0, 16) + "..." : line) + "\r\n\r\n");
			}
            line = null;
        }
        return line;
    ...
    private StringBuilder writeSections(StringBuilder w, Section section) throws IOException {
        ...
        if (!section.getName().isEmpty()) {
			if (section.getData() != null || !section.getChilds().isEmpty()) {
				w.append(space + section.getName() + ": ");
			}
            if (section.getData() != null) {
                switch (section.getType()) {
                    case DECIMAL:
                        w.append(section.getDouble().toString());
                        break;
                    case INTEGER:
                        w.append(section.getLong().toString());
                        break;
                    case LIST:
                        List<String> list = section.getStringList();
						if (list.isEmpty()) {
							...
							for (String str : list) {
								...
								int pos = 0;
								while ((pos = str.indexOf(' ', pos + 64)) != -1 && (pos == str.length() - 1 || str.charAt(pos + 1) != ' ')) {
                    ...
                    case STRING:
						...
						else if (text.startsWith(" ") || text.endsWith(" ") || text.contains(":")) {
							text = "'" + text + "'";
						}
                        int pos = 0;
						while ((pos = text.indexOf(' ', pos + 64)) != -1 && (pos == text.length() - 1 || text.charAt(pos + 1) != ' ')) {
		...
		for (int i = space.length(); i < section.getLevel() * 2; i++) {
			space += " ";
		}
        for (Data data : section.getChilds()) {
			if (data.getType() == Type.COMMENT) {
				w.append(space + (String) ((Comment) data).getData() + "\n");
			}
			else if (data.getType() == Type.SPACE) {
				w.append(space + "\n");
			}
			else {
				writeSections(w, (Section) data);
			}
        }
        return w;
    }
