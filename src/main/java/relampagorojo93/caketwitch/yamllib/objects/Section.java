...
public class Section extends Data {
    ...
    public Section(Section parent, String name, Object data) {
    ...
    public String getPath() {
        String path = name;
		for (Section parent = this.parent; parent != null && parent.getLevel() != 0; parent = parent.getParent()) {
    ...
    public Section getNonNullChild(String name) {
        return getChild(name, new Section());
    }

    public Section getChild(String name) {
        return getChild(name, (Section) null);
    }
    ...
    public Section getChild(String name, Section def) {
		for (Data data : childs) {
			if (data instanceof Section && ((Section) data).getName().equals(name)) {
				return (Section) data;
			}
		}
        return def;
