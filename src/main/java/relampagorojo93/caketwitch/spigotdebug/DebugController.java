...
public class DebugController {
    ...
    public void addDebugData(DebugData data) {
		if (++currentdata >= this.data.length) {
			currentdata = 0;
		}
    ...
    public DebugData getDebugData(int index) {
		if (index < 0 || index >= this.data.length) {
			throw new ArrayIndexOutOfBoundsException("Index: " + index + ", length: " + this.data.length);
		}
		if (index == -1) {
			return null;
		}
    ...
    public void changeDebugLength(int size) {
		...
		for (int i = 0; i < size && i < getDebugLength(); i++) {
			newdata[i] = getDebugData(i);
		}
    }
    ...
    public String toString() {
		...
		for (DebugData dd : data) {
			sb.append("[" + new Date(dd.getLogTime()) + "] " + dd.getDebugType().name() + " " + dd.getMessage()).append("\r\n");
		}
        return sb.toString();
    }
}
